...
- **An account's gym lives in `app_metadata.gym_id`**, set by whoever creates it; the auth trigger reads it to create or link the person row. No `gym_id`, no profile. Pre-tenancy accounts got it from their person row (`20260925040000`); `setTemporaryPassword`/`revokeAccess` refuse an account whose `gym_id` is not the caller's, or a platform admin.
