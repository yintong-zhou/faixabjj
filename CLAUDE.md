...
**Only one age minimum is seeded, and it is 18.** `belt-criteria.md` merges the IBJJF _General System of Graduation_ with common academy practice and declares the federation the higher authority, so where the two disagree the IBJJF figure wins. The black belt is where they did: the community summary said 19, the IBJJF says 18, and `20260919000000_black_belt_min_age.sql` corrects the value `20260918100000` had already seeded — a corrective migration, because that one was applied. The document also carries IBJJF ages for the three lower transitions (16 for blue, 16 for purple, 18 for brown); they are deliberately **not** seeded, and the corrective migration explains why at its foot. Seeding them is not a typo fix but a change to who the queue is for: `promotionStatus()` reads `min_age_years` generically, so an age on those rows drops every younger member out of the count, the `idonei` filter and the green dot at once — and takes with them anyone whose `birth_date` is blank, since an unknown age is never a passed check.
...
- **Belts are drawn, not spelled out.** `components/belt.tsx` renders the belt graphic wherever a rank is shown — Registro list and detail, `/account`, the roll call. Use it instead of printing the colour and a stripe count; the two are one thing on a belt. Three deliberate points:
  - Artwork lives in `frontend/public/belts/<colour>-<n>-stripe.png` (1000×300, transparent), named after the `belt_rank` enum value, so the enum value *is* the filename prefix and no mapping table stands between them. The purple files used to be called `violet-*` with a `BELT_FILE_COLOR` map bridging the two vocabularies; they were renamed instead. Keep new artwork named after the enum. `MAX_STRIPES` is now the list of colours that have artwork at all (black goes to 7 for its degrees, everyone else to 4): a belt with no entry falls back to the words rather than a broken image.
